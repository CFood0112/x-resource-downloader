...
if (Test-Path $runtimeDir) {
    $runtimeTemp = Join-Path $root 'release\runtime_temp'
    if (Test-Path $runtimeTemp) {
        Remove-Item -LiteralPath $runtimeTemp -Recurse -Force
    }
    Move-Item -LiteralPath $runtimeDir -Destination $runtimeTemp
}
...
if (Test-Path $runtimeTemp) {
    Move-Item -LiteralPath $runtimeTemp -Destination $runtimeDir
}
